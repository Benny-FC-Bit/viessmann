...
class VitoConnect extends WebHookModule
{
    ...
    public function RequestAction($Ident, $Value)
    {
        ...
        switch ($name) {
            case 'active':
                if ($Value) {
                    $this->RequestDeviceData($id . '/commands/activate');
                } else {
                    $this->RequestDeviceData($id . '/commands/deactivate');
                }
                $this->SetValue($Ident, $Value);
                break;
            case 'value':
                if (strpos($Ident, 'modes') !== false) {
                    $this->RequestDeviceData($id . '/commands/setMode', [
                        'mode' => $Value
                    ]);
                    $this->SetValue($Ident, $Value);
                } elseif (strpos($Ident, 'hysteresis') !== false) {
                    $this->RequestDeviceData($id . '/commands/setHysteresis', [
                        'hysteresis' => $Value
                    ]);
                    $this->SetValue($Ident, $Value);
                } elseif (strpos($Ident, 'temperature') !== false) {
                    $this->RequestDeviceData($id . '/commands/setTargetTemperature', [
                        'temperature' => $Value
                    ]);
                    $this->SetValue($Ident, $Value);
                } else {
                ...
                break;
            case 'temperature':
                $this->RequestDeviceData($id . '/commands/setTemperature', [
                    'targetTemperature' => $Value
                ]);
                $this->SetValue($Ident, $Value);
                break;
            case 'min':
                $this->RequestDeviceData($id . '/commands/setMin', [
                    'temperature' => $Value
                ]);
                $this->SetValue($Ident, $Value);
                break;
            case 'max':
                $this->RequestDeviceData($id . '/commands/setMax', [
                    'temperature' => $Value
                ]);
                $this->SetValue($Ident, $Value);
                break;
...
public function CreateZirku($start,$end,$activate)
    {
		//https://api.viessmann-climatesolutions.com/iot/v2/features/installations/{{installationID}}/gateways/{{gatewaySerial}}/devices/{{deviceId}}/features/heating.circuits.0.heating.schedule/commands/setSchedule
		...
		return $this->SendAction(sprintf($this->feature_data_url, $id, $serial) . $action, $schedule);
